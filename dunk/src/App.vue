<template>
  <div id="app" class="app-container">
    ...
    <header class="topbar" :class="{ collapsed }" :style="{ '--sidebar-current-width': collapsed ? 'var(--sidebar-collapsed-width)' : 'var(--sidebar-width)' }">
    ...
    <main :class="['main-content', { 'collapsed': collapsed }]" :style="{ '--sidebar-current-width': collapsed ? 'var(--sidebar-collapsed-width)' : 'var(--sidebar-width)' }">
      <router-view />
    </main>
  </div>
</template>
...
.main-content.collapsed { /* fallback for older browsers */
...
.top-logo { width:44px; height:44px; display:flex; align-items:center; justify-content:center; color:#ffb14d }
.top-logo svg { width:28px; height:28px }
.top-title { color: orange; font-size: 1.6rem; font-weight:800 }
.create-btn { background:#ff9a3c; color:#111; padding:10px 16px; border-radius:8px; border:none }
